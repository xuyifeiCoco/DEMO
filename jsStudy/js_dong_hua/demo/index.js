...
function repeat() {
	var repeatAnimation = animation().loadImage(firstImage).changePosition($rabbit1, rightRunningMap, images[0]).repeatForever();
	// var repeatAnimation = animation().loadImage(firstImage).changePosition($rabbit1, rightRunningMap, images[0]).repeat(1);
...
function run() {
  ...
  var a=1
    ...
    // $rabbit2.style.backgroundImage = 'url(' + a + ')';
...
function lose() {
  // var loseAnimation=animation().loadImage(images).changePosition($rabbit4, rabbitLoseMap, images[1]).repeatForever()
